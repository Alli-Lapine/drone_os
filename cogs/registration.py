...
class Registration(commands.Cog):
    ...
    registration = SlashCommandGroup(
        "registration", "Manage connection to the hive", guild_ids=guilds
    )

    @registration.command(
        name="connect", guild_ids=guilds, description="Register yourself as a drone"
    )
    async def register_drone(
        ...
        if len(drone_id) != 4:
            await ctx.respond(
                embed=mkembed("error", f"`Drone IDs must be exactly 4 characters long`")
            )
            return
    ...
    @registration.command(
        name="disconnect",
        description="Disconnect yourself from the hive, regaining full autonomy",
    ...
    async def disconnect_drone(self, ctx: discord.ApplicationContext):
        ...
        if not drone:
            await ctx.respond(
                embed=mkembed("error", "`You do not appear to be a registered drone.`")
            )
            return
    ...
    async def sethive(
        self, ctx: discord.ApplicationContext, hive: Option(str, choices=util.fhivemap)
    ):
        await ctx.defer()
        drone = get_drone(ctx.author.id)
        if not drone:
        ...
        await ctx.respond(
            embed=mkembed(
                "done", f"Your hive set to {actual_hive} ({util.hivemap[actual_hive]['sym']})"
            )
        )
