...
{message}```"""
    ...
    @commands.slash_command(name='dc', description='Assume direct control of another drone', guild_ids=guilds)
    async def ssh(self, ctx: ApplicationContext, drone: Option(str, "Drone ID", required=True)):
        ...
        if operator.droneid == target.droneid:
            await ctx.respond(embed=mkembed('error', '`You are already in direct control of yourself (hopefully)`'))
            return
        oconf = operator.get('config', {})
        if oconf.get('ssh', False) is not False:
            await ctx.respond(embed=mkembed('error', f'`ssh: Max nesting level reached, disconnect from {oconf["ssh"]} first'))
            return
        operator['config'] = oconf
        operator['config']['ssh'] = target.droneid
        Storage.backend.save(operator)
        await ctx.respond(embed=mkembed('done', f'`{operator.droneid}@droneOS $ ssh {operator.droneid}@{target.droneid}\n'
    ...
    @commands.slash_command(name='dc-end', description='Relinquish direct control of another drone', guild_ids=guilds)
    async def sshend(self, ctx: ApplicationContext):
        operator, target, error = await get_command_drones(ctx.author.id, ctx.author.id, chkaccess=False)
        if error:
            await ctx.respond(embed=error)
            return
        oconf = operator.get('config', {})
