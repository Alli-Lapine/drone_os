...
from util.access_utils import (
    grant_access,
    revoke_access,
    has_access,
    accesslist_to_dronelist,
    get_drone_hive_owner,
)
from util.storage import get_drone


class Access(commands.Cog):
    accessgrp = SlashCommandGroup(
        name="access", description="Drone access management", guild_ids=guilds
    )
    ...
    @accessgrp.command(
        name="add", description="Add a drone to another drone's access list", guild_ids=guilds
    )
    async def add_access(
        ...
        if not has_access(operator, target_drone):
            await ctx.respond(
                embed=mkembed(
                    "error",
                    f"`{operator.droneid}@droneOS $ touch /dev/{target_drone.droneid}/access/{to_add_drone.droneid}\ntouch: permission denied`",
                )
            )
            return
        else:
            ...
            if not r:
                await ctx.respond(
                    embed=mkembed(
                        "error",
                        f"`{to_add_drone.droneid} is already on the access list of {target_drone.droneid}`",
                    )
                )
            else:
                await ctx.respond(
                    embed=mkembed(
                        "done",
                        f"`{to_add_drone.droneid} added to the access list of {target_drone.droneid}`",
                    )
                )

    @accessgrp.command(
        name="rm", description="Remove a drone from another drone's access list", guild_ids=guilds
    )
    async def rm_access(
        self,
        ctx: ApplicationContext,
        ...
        if not has_access(operator, target_drone):
            await ctx.respond(
                embed=mkembed(
                    "error",
                    f"`{operator.droneid}@droneOS $ rm /dev/{target_drone.droneid}/access/{to_rm_drone.droneid}\nrm: permission denied`",
                )
            )
            return
        else:
            ...
            if not r:
                ...
                await ctx.respond(
                    embed=mkembed(
                        "done",
                        f"`{to_rm_drone.droneid} removed from the access list of {target_drone.droneid}`",
                    )
                )
    ...
    async def ls_access(
        self,
        ctx: ApplicationContext,
        target: Option(str, description="Drone to check (yourself if blank)", required=False),
    ):
        operator = get_drone(ctx.author.id)
        if not operator:
            await ctx.respond(embed=mkembed("error", "`You do not appear to be a drone.`"))
        ...
        if not has_access(operator, target_drone):
            await ctx.respond(
                embed=mkembed(
                    "error",
                    f"`{operator.droneid}@droneOS $ ls /dev/{target_drone.droneid}/access/\nls: permission denied`",
                )
            )
            return
        else:
