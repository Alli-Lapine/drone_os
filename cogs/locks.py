...
class Locks(commands.Cog):
    ...
    @locksgrp.command(name='prefix-on',
                      description="Lock the specified drone into prefix chat",
                      guild_ids=guilds)
    async def lock_drone(self, ctx: ApplicationContext,
                         droneid: Option(str, description="The drone ID being locked (yourself if blank)", required=False),
        ...
        await ctx.respond(
            embed=mkembed('done', f'`Drone {target["droneid"]} prefix chat has been locked{msgtime}.`'))
        return

    @locksgrp.command(name='prefix-off', description="Unlock the specified drone's prefix requirement", guild_ids=guilds)
    async def unlock_drone(self, ctx: ApplicationContext,
                           droneid: Option(str, description="The drone ID being unlocked (yourself if blank)", required=False)):
        await ctx.defer()
        operator, target, error = await get_command_drones(ctx.author.id, droneid or ctx.author.id)
        if error:
            await ctx.respond(embed=error)
    ...
    @locksgrp.command(name='prefix-channel-on',
    ...
    async def lock_channel(self, ctx: ApplicationContext,
        ...
        await ctx.respond(embed=mkembed('done',
                                        f'Speech optimizations locked for {"EVERYONE" if lockall else "drones"} in {chan.mention}'))

    @locksgrp.command(name='prefix-channel-off', description="Unlock prefix chat in the given channel", guild_ids=guilds)
    async def unlock_channel(self, ctx: ApplicationContext,
                             channel: Option(discord.TextChannel, required=False)):
        await ctx.defer()
        chan = channel or ctx.channel
        db_chan = get_channel({'discordid': chan.id})
