...
@pytest.fixture
def enforcedrone():
    drone1 = RegisteredDrone({'discordid': 100000000000000041, 'droneid': "0041", 'config': {'enforce': True}})
    Storage.backend.save(drone1)
...
class TestFilterLogic:
    async def test_sends_normal_msg(self, filterplugin, hook, msg, normaldrone):
        ...
        hook.send.assert_called_with(username=ANY, content="0001 :: ☼ :: Y helo thar", avatar_url=ANY)
    ...
    async def test_deletes_unprefixed_from_drone_in_droneenforce(self, filterplugin, hook, msg, enforcedrone):
        msg.configure_mock(content="Y helo thar")
    ...
    async def test_sends_in_droneenforce(self, filterplugin, hook, msg, enforcedrone):
        ...
        msg.author.id = enforcedrone
        await filterplugin.drone_filter_handler(msg, hook)
        hook.send.assert_called_with(username=ANY, content="0041 :: ☼ :: Y helo thar", avatar_url=ANY)
        msg.delete.assert_called()

    async def test_sends_in_chanenforce(self, filterplugin, hook, msg, normaldrone, enforcedronechan):
        msg.configure_mock(content="0001 :: Y helo thar")
        msg.author.id = normaldrone
    ...
    async def test_deletes_unprefixed_from_drone_in_chanenforce(self, filterplugin, hook, msg, normaldrone, enforcedronechan):
        msg.configure_mock(content="Y helo thar")
        msg.author.id = normaldrone
        msg.channel.id = enforcedronechan
        await filterplugin.drone_filter_handler(msg, hook)
    ...
    async def test_deletes_unprefixed_from_drone_in_chanenforceall(self, filterplugin, hook, msg, normaldrone, enforceallchan):
        msg.configure_mock(content="Y helo thar")
        msg.author.id = normaldrone
    ...
    async def test_deletes_unprefixed_from_normie_in_chanenforceall(self, filterplugin, hook, msg, enforceallchan):
        msg.configure_mock(content="Y helo thar")
        msg.author.id = 100000000000000999
        msg.channel.id = enforceallchan
        await filterplugin.drone_filter_handler(msg, hook)
    ...
    async def test_sends_in_chanenforceall(self, filterplugin, hook, msg, normaldrone, enforceallchan):
        msg.configure_mock(content="0001 :: Y helo thar")
        msg.author.id = normaldrone
        msg.channel.id = enforceallchan
        await filterplugin.drone_filter_handler(msg, hook)
